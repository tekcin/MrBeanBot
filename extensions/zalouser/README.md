...
MrBeanBot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
MrBeanBot channels login --channel zalouser
...
```bash
MrBeanBot message send --channel zalouser --target <threadId> --message "Hello from MrBeanBot!"
```
...
```bash
MrBeanBot channels login --channel zalouser              # Login via QR
MrBeanBot channels login --channel zalouser --account work
MrBeanBot channels status --probe
MrBeanBot channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
MrBeanBot directory self --channel zalouser
MrBeanBot directory peers list --channel zalouser --query "name"
MrBeanBot directory groups list --channel zalouser --query "work"
MrBeanBot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
MrBeanBot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
MrBeanBot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `MrBeanBot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
MrBeanBot channels login --channel zalouser --account work
MrBeanBot message send --channel zalouser --account work --target <id> --message "Hello"
