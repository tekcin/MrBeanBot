...
export function listGoogleChatAccountIds(cfg: MrBeanBotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: MrBeanBotConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: MrBeanBotConfig): ResolvedGoogleChatAccount[] {
