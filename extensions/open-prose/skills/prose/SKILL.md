...
metadata: {"MrBeanBot":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == MrBeanBot `sessions_spawn`
- **File I/O** == MrBeanBot `read`/`write`
- **Remote fetch** == MrBeanBot `web_fetch` (or `exec` with curl when POST is required)
