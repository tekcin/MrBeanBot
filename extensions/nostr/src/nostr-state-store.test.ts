...
import type { PluginRuntime } from "MrBeanBot/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.MRBEANBOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-nostr-"));
  process.env.MRBEANBOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
  ...
  try {
    ...
    if (previous === undefined) delete process.env.MRBEANBOT_STATE_DIR;
    else process.env.MRBEANBOT_STATE_DIR = previous;
