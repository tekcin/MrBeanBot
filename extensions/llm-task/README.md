...
This extension depends on MrBeanBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** MrBeanBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.MrBeanBot/extensions` as a standalone plugin directory.
