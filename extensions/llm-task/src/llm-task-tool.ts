...
// NOTE: This extension is intended to be bundled with MrBeanBot.
// When running from source (tests/dev), MrBeanBot internals live under src/.
...
export function createLlmTaskTool(api: MrBeanBotPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-llm-task-"));
