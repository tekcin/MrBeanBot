...
export function listBlueBubblesAccountIds(cfg: MrBeanBotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: MrBeanBotConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: MrBeanBotConfig): ResolvedBlueBubblesAccount[] {
