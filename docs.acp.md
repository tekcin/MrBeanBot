# MrBeanBot ACP Bridge

This document describes how the MrBeanBot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`MrBeanBot acp` exposes an ACP agent over stdio and forwards prompts to a running
MrBeanBot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a MrBeanBot Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `MrBeanBot acp` over stdio.

Example config:

```bash
MrBeanBot config set gateway.remote.url wss://gateway-host:18789
MrBeanBot config set gateway.remote.token <token>
```

Example run:

```bash
MrBeanBot acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
MrBeanBot acp --session agent:main:main
MrBeanBot acp --session agent:design:main
MrBeanBot acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “MrBeanBot ACP” to start a thread.

## Execution Model

- ACP client spawns `MrBeanBot acp` and speaks ACP messages over stdio.
...
```bash
MrBeanBot acp --session agent:main:main
MrBeanBot acp --session-label "support inbox"
MrBeanBot acp --reset-session
```
...
`MrBeanBot acp` resolves the Gateway URL and auth from CLI flags or config:
