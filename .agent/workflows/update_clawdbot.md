---
description: Update MrBeanBot from upstream when branch has diverged (ahead/behind)
---

# MrBeanBot Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "MrBeanBot" || true

# Move old version
mv /Applications/MrBeanBot.app /tmp/MrBeanBot-backup.app

# Install new build
cp -R dist/MrBeanBot.app /Applications/

# Launch
open /Applications/MrBeanBot.app
```
...
pnpm MrBeanBot health

# Verify no zombie processes
ps aux | grep -E "(MrBeanBot|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm MrBeanBot agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update MrBeanBot.json with fallback chains
...
pnpm MrBeanBot agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
