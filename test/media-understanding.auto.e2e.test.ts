...
import type { MrBeanBotConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("MrBeanBot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("MrBeanBot-bin-sherpa-");
      const modelDir = await makeTempDir("MrBeanBot-sherpa-model-");
      ...
      const cfg: MrBeanBotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("MrBeanBot-bin-whispercpp-");
      const modelDir = await makeTempDir("MrBeanBot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("MrBeanBot-bin-gemini-");
      ...
      const cfg: MrBeanBotConfig = { tools: { media: { image: {} } } };
