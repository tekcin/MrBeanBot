...
private let webChatSwiftLogger = Logger(subsystem: "com.tekcin.mrbeanbot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: MrBeanBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MrBeanBotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(MrBeanBotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MrBeanBotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MrBeanBotGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    MrBeanBotChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    MrBeanBotAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MrBeanBotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MrBeanBotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MrBeanBotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: MrBeanBotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MrBeanBotChatView>) -> NSViewController
    {
