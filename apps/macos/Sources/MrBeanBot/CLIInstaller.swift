...
enum CLIInstaller {
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing MrBeanBot CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed MrBeanBot \($0)." } ?? "Installed MrBeanBot."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://mrbeanbot.com/install-cli.sh | \
