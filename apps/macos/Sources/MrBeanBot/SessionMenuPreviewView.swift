...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MrBeanBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MrBeanBotSessionPreviewEntry,
    ...
    private static func cache(payload: MrBeanBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MrBeanBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MrBeanBotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MrBeanBotKit.AnyCodable]) -> [MrBeanBotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MrBeanBotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MrBeanBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MrBeanBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MrBeanBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MrBeanBotChatMessage) -> String? {
