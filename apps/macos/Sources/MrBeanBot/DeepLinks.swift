...
private let deepLinkLogger = Logger(subsystem: "com.tekcin.mrbeanbot", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "MrBeanBot is paused", message: "Unpause MrBeanBot to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run MrBeanBot agent?", message: body) else { return }
        }
