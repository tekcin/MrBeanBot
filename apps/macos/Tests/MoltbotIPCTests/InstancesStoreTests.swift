...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: MrBeanBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MrBeanBotProtocol.AnyCodable] = [
            "presence": .init([MrBeanBotProtocol.AnyCodable(entry)]),
        ]
        let payload = MrBeanBotProtocol.AnyCodable(payloadMap)
