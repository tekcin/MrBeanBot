...
struct IOSGatewayChatTransport: MrBeanBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MrBeanBotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MrBeanBotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MrBeanBotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MrBeanBotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MrBeanBotChatAttachmentPayload]) async throws -> MrBeanBotChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MrBeanBotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(MrBeanBotChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(MrBeanBotGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MrBeanBotGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MrBeanBotChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MrBeanBotAgentEventPayload.self)
                        {
