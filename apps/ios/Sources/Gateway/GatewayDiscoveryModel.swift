...
final class GatewayDiscoveryModel {
    ...
    func start() {
        ...
        for domain in MrBeanBotBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: MrBeanBotBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "com.tekcin.mrbeanbot.ios.gateway-discovery.\(domain)"))
        }
    }
    ...
    private static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (MrBeanBot)", with: "")
