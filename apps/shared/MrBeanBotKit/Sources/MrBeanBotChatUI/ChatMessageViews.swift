...
struct ChatMessageBubble: View {
    let message: MrBeanBotChatMessage
    let style: MrBeanBotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? MrBeanBotChatTheme.userText : MrBeanBotChatTheme.assistantText
    ...
    private var inlineAttachments: [MrBeanBotChatMessageContent] {
    ...
    private var toolCalls: [MrBeanBotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [MrBeanBotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? MrBeanBotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return MrBeanBotChatTheme.onboardingAssistantBubble
        }
        return MrBeanBotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return MrBeanBotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: MrBeanBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? MrBeanBotChatTheme.userText : MrBeanBotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: MrBeanBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MrBeanBotChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(MrBeanBotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [MrBeanBotChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: MrBeanBotChatTheme.assistantText)
            }
        }
    }
}
