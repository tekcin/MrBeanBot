...
# Build and bundle MrBeanBot into a minimal .app we can open.
# Outputs to dist/MrBeanBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/MrBeanBot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="MrBeanBot"
BUNDLE_ID="${BUNDLE_ID:-com.tekcin.mrbeanbot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/MrBeanBot/MrBeanBot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/MrBeanBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MrBeanBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MrBeanBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/MrBeanBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/MrBeanBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/MrBeanBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/MrBeanBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/MrBeanBot/Resources/MrBeanBot.icns" "$APP_ROOT/Contents/Resources/MrBeanBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/MrBeanBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MrBeanBotKit resources"
MRBEANBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MrBeanBotKit_MrBeanBotKit.bundle"
if [ -d "$MRBEANBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MrBeanBotKit_MrBeanBotKit.bundle"
  cp -R "$MRBEANBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MrBeanBotKit_MrBeanBotKit.bundle"
else
  echo "WARN: MrBeanBotKit resource bundle not found at $MRBEANBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running MrBeanBot"
killall -q MrBeanBot 2>/dev/null || true
