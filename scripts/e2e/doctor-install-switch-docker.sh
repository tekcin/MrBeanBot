...
IMAGE_NAME="MrBeanBot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/MrBeanBot-bin:$PATH"
  mkdir -p /tmp/MrBeanBot-bin

  cat > /tmp/MrBeanBot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/MrBeanBot-bin/systemctl

  cat > /tmp/MrBeanBot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/MrBeanBot-bin/loginctl
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/MrBeanBot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/MrBeanBot-gateway.service"
    if [ ! -f "$unit_path" ]; then
