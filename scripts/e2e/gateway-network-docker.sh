...
IMAGE_NAME="MrBeanBot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="MrBeanBot-net-e2e-$$"
GW_NAME="MrBeanBot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "MRBEANBOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "MRBEANBOT_SKIP_CHANNELS=1" \
	  -e "MRBEANBOT_SKIP_GMAIL_WATCHER=1" \
	  -e "MRBEANBOT_SKIP_CRON=1" \
	  -e "MRBEANBOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
