...
IMAGE_NAME="MrBeanBot-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/MrBeanBot-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.MrBeanBot/extensions"

  cat > "$HOME/.MrBeanBot/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/MrBeanBot-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@MrBeanBot/demo-plugin-tgz",
  "version": "0.0.1",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/MrBeanBot-plugin-dir.XXXXXX")"
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/MrBeanBot-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@MrBeanBot/demo-plugin-file",
  "version": "0.0.1",
  "MrBeanBot": { "extensions": ["./index.js"] }
}
JSON
