...
IMAGE_NAME="MrBeanBot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/MrBeanBot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/MrBeanBot-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/MrBeanBot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/MrBeanBot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/MrBeanBot-e2e-$1.XXXXXX"
  }
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.MrBeanBot/MrBeanBot.json" <<'"'"'JSON'"'"'
{
