...
# MrBeanBot Auth Widget for Termux
...
SERVER="${MRBEANBOT_SERVER:-l36}"

# Check auth status
termux-toast "Checking MrBeanBot auth..."

STATUS=$(ssh "$SERVER" '$HOME/MrBeanBot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/MrBeanBot/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|MRBEANBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/MrBeanBot/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|MRBEANBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: MrBeanBot won't work until re-auth"
                ;;
        esac
        ;;
