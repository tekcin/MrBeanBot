...
const compiler = env.MRBEANBOT_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.MRBEANBOT_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.MRBEANBOT_RUNNER_LOG === "0") return;
  process.stderr.write(`[MrBeanBot] ${message}\n`);
};
