...
ZIP=${1:?"Usage: $0 MrBeanBot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/MrBeanBot/MrBeanBot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^MrBeanBot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/MrBeanBot/MrBeanBot/releases/download/v${VERSION}/"}
