...
#
# Suggested cron: */30 * * * * /home/admin/MrBeanBot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send MrBeanBot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., MrBeanBot-alerts)
...
STATE_FILE="$HOME/.MrBeanBot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via MrBeanBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use MrBeanBot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via MrBeanBot to $NOTIFY_PHONE..."
            MrBeanBot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! MrBeanBot is down. Run: ssh l36 '~/MrBeanBot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
