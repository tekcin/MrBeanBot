...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/MrBeanBot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "MrBeanBot Re-Auth" -c "After getting key, run: ssh $SERVER '~/MrBeanBot/scripts/mobile-reauth.sh'" --id mrbeanbot-auth
        ;;
    *)
