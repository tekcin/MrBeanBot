...
BASE_IMAGE="${BASE_IMAGE:-MrBeanBot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-MrBeanBot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=MrBeanBot.sandbox=1)
NOTE
