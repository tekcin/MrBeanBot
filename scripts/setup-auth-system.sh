#!/bin/bash
# Setup MrBeanBot Auth Management System
...
echo "=== MrBeanBot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'MrBeanBot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  MrBeanBot message: Send warning via MrBeanBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/MrBeanBot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/MrBeanBot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/MrBeanBot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now MrBeanBot-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/MrBeanBotAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/MrBeanBotAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/MrBeanBot*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (MrBeanBotAuth) shows status and opens auth URL if needed."
echo "The full widget (MrBeanBotAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status MrBeanBot-auth-monitor.timer"
echo ""
echo "Quick commands:"
