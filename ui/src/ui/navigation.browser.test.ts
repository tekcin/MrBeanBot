...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/MrBeanBot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/MrBeanBot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/MrBeanBot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MRBEANBOT_CONTROL_UI_BASE_PATH__ = "/MrBeanBot";
    const app = mountApp("/MrBeanBot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/MrBeanBot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/MrBeanBot/sessions");
  });
