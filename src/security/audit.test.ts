...
describe("security audit", () => {
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: MrBeanBotConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-security-audit-discord-"));
    process.env.MRBEANBOT_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) delete process.env.MRBEANBOT_STATE_DIR;
      else process.env.MRBEANBOT_STATE_DIR = prevStateDir;
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "MrBeanBot-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.MRBEANBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.MRBEANBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-security-audit-discord-open-"));
    process.env.MRBEANBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MrBeanBotConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.MRBEANBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-security-audit-slack-"));
    process.env.MRBEANBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MrBeanBotConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.MRBEANBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-security-audit-slack-open-"));
    process.env.MRBEANBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MrBeanBotConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.MRBEANBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-security-audit-telegram-"));
    process.env.MRBEANBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MrBeanBotConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: MrBeanBotConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.MRBEANBOT_GATEWAY_TOKEN;
    process.env.MRBEANBOT_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: MrBeanBotConfig = {
    ...
    try {
      ...
      if (prevToken === undefined) delete process.env.MRBEANBOT_GATEWAY_TOKEN;
      else process.env.MRBEANBOT_GATEWAY_TOKEN = prevToken;
    }
  });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: MrBeanBotConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.MRBEANBOT_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.MRBEANBOT_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.MRBEANBOT_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.MRBEANBOT_GATEWAY_PASSWORD;
      } else {
        process.env.MRBEANBOT_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.MRBEANBOT_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: MrBeanBotConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.MRBEANBOT_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: MrBeanBotConfig = {
        gateway: {
          mode: "remote",
