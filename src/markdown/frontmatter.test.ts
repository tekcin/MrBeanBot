...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { MrBeanBot?: { emoji?: string } };
    expect(parsed.MrBeanBot?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { MrBeanBot?: { events?: string[] } };
