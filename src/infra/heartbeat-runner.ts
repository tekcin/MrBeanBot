...
function resolveActiveHoursTimezone(cfg: MrBeanBotConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: MrBeanBotConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: MrBeanBotConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: MrBeanBotConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: MrBeanBotConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: MrBeanBotConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: MrBeanBotConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: MrBeanBotConfig) => {
    if (state.stopped) return;
    const now = Date.now();
