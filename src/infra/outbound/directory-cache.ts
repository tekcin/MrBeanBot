...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MrBeanBotConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: MrBeanBotConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MrBeanBotConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: MrBeanBotConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: MrBeanBotConfig): void {
