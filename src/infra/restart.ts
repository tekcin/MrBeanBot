...
export function triggerMrBeanBotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.MRBEANBOT_SYSTEMD_UNIT,
        process.env.MRBEANBOT_PROFILE,
      );
  ...
  const label =
    process.env.MRBEANBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MRBEANBOT_PROFILE);
