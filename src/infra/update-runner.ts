...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "MrBeanBot doctor",
        managerScriptArgs(manager, "MrBeanBot", ["doctor", "--non-interactive"]),
        gitRoot,
        { MRBEANBOT_UPDATE_IN_PROGRESS: "1" },
      ),
    );
