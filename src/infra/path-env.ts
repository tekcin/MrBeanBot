...
function candidateBinDirs(opts: EnsureMrBeanBotPathOpts): string[] {
  ...
  // Bundled macOS app: `MrBeanBot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingMrBeanBot = path.join(execDir, "MrBeanBot");
    if (isExecutable(siblingMrBeanBot)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/MrBeanBot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "MrBeanBot"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `MrBeanBot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMrBeanBotCliOnPath(opts: EnsureMrBeanBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MRBEANBOT_PATH_BOOTSTRAPPED)) return;
  process.env.MRBEANBOT_PATH_BOOTSTRAPPED = "1";
