...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "MrBeanBot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "MrBeanBot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "MrBeanBot-gw");
