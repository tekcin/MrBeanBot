...
import type { MrBeanBotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareMrBeanBotVersions } from "./version.js";
...
function coerceConfig(value: unknown): MrBeanBotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as MrBeanBotConfig;
}
...
function stampConfigVersion(cfg: MrBeanBotConfig): MrBeanBotConfig {
...
function warnIfConfigFromFuture(cfg: MrBeanBotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareMrBeanBotVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer MrBeanBot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: MrBeanBotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MrBeanBotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as MrBeanBotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as MrBeanBotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: MrBeanBotConfig) {
    clearConfigCache();
...
// module scope. `MRBEANBOT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MRBEANBOT_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MRBEANBOT_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): MrBeanBotConfig {
...
export async function writeConfigFile(cfg: MrBeanBotConfig): Promise<void> {
  clearConfigCache();
