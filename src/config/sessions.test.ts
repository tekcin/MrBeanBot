...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-mrbeanbot",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-mrbeanbot#general");
  });
  ...
  it("derives session transcripts dir from MRBEANBOT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MRBEANBOT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.MRBEANBOT_STATE_DIR = "/custom/state";
    try {
