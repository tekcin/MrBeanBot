import type { MrBeanBotConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: MrBeanBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: MrBeanBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: MrBeanBotConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: MrBeanBotConfig): boolean {
...
function isIMessageConfigured(cfg: MrBeanBotConfig): boolean {
...
function isWhatsAppConfigured(cfg: MrBeanBotConfig): boolean {
...
function isGenericChannelConfigured(cfg: MrBeanBotConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: MrBeanBotConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: MrBeanBotConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: MrBeanBotConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: MrBeanBotConfig, pluginId: string): boolean {
...
function enablePluginEntry(cfg: MrBeanBotConfig, pluginId: string): MrBeanBotConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
