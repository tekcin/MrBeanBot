...
import type { MrBeanBotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: MrBeanBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: MrBeanBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MrBeanBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MrBeanBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MrBeanBotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as MrBeanBotConfig)),
    ),
  };
}
