...
import type { MrBeanBotPluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: MrBeanBotPluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: MrBeanBotPluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
