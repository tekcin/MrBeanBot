...
2. **Appends to log file** - Writes a JSON line to `~/.MrBeanBot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.MrBeanBot/logs/commands.log ~/.MrBeanBot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/MrBeanBot`:
   ```
   /home/username/.MrBeanBot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
