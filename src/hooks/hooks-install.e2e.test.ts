...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevBundledDir = process.env.MRBEANBOT_BUNDLED_HOOKS_DIR;
    process.env.MRBEANBOT_STATE_DIR = path.join(baseDir, "state");
    process.env.MRBEANBOT_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.MRBEANBOT_BUNDLED_HOOKS_DIR;
    } else {
      process.env.MRBEANBOT_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          MrBeanBot: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
