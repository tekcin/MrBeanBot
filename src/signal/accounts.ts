...
export function listSignalAccountIds(cfg: MrBeanBotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: MrBeanBotConfig): string {
...
function mergeSignalAccountConfig(cfg: MrBeanBotConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: MrBeanBotConfig): ResolvedSignalAccount[] {
