...
export function listSlackAccountIds(cfg: MrBeanBotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: MrBeanBotConfig): string {
...
function mergeSlackAccountConfig(cfg: MrBeanBotConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: MrBeanBotConfig): ResolvedSlackAccount[] {
