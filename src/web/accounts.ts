...
export function listWhatsAppAuthDirs(cfg: MrBeanBotConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: MrBeanBotConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: MrBeanBotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: MrBeanBotConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: MrBeanBotConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: MrBeanBotConfig): ResolvedWhatsAppAccount[] {
