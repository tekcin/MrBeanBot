...
import { scheduleGatewaySigusr1Restart, triggerMrBeanBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting MrBeanBot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMrBeanBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting MrBeanBot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
