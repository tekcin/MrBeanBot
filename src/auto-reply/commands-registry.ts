import type { MrBeanBotConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: MrBeanBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MrBeanBotConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: MrBeanBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MrBeanBotConfig) {
