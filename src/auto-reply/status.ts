...
type AgentConfig = Partial<NonNullable<NonNullable<MrBeanBotConfig["agents"]>["defaults"]>>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 MrBeanBot ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: MrBeanBotConfig): string {
  const lines = ["ℹ️ Help", ""];
