...
/**
 * Cleanup suspended MrBeanBot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: MrBeanBotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-cli-images-"));
  const paths: string[] = [];
