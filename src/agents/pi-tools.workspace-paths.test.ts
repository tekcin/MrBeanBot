...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("MrBeanBot-ws-", async (workspaceDir) => {
      await withTempDir("MrBeanBot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createMrBeanBotCodingTools({ workspaceDir });
    ...
    await withTempDir("MrBeanBot-ws-", async (workspaceDir) => {
      await withTempDir("MrBeanBot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello MrBeanBot");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("MrBeanBot-ws-", async (workspaceDir) => {
      await withTempDir("MrBeanBot-override-", async (overrideDir) => {
        const tools = createMrBeanBotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("MrBeanBot-sandbox-", async (sandboxDir) => {
      await withTempDir("MrBeanBot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "MrBeanBot-sbx-test",
        ...
        const tools = createMrBeanBotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
