...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-oauth-"));

    try {
      process.env.MRBEANBOT_STATE_DIR = tempDir;
      process.env.MRBEANBOT_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.MRBEANBOT_AGENT_DIR, {
