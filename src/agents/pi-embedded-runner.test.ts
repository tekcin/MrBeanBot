...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-embedded-agent-"));
...
const ensureModels = (cfg: MrBeanBotConfig) => ensureMrBeanBotModelsJson(cfg, agentDir) as unknown;
