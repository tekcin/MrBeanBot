...
function resolveDefaultModelRef(cfg?: MrBeanBotConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: MrBeanBotConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: MrBeanBotConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureMrBeanBotModelsJson(effectiveCfg, params.agentDir);
