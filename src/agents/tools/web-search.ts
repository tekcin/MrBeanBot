...
type WebSearchConfig = NonNullable<MrBeanBotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: MrBeanBotConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("MrBeanBot configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.mrbeanbot.com/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://mrbeanbot.com",
      "X-Title": "MrBeanBot Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
