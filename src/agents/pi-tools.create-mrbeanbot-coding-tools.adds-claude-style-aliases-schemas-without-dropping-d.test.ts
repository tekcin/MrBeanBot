...
describe("createMrBeanBotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from MrBeanBot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "MrBeanBot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "MrBeanBot-workspace"),
    ...
    const tools = createMrBeanBotCodingTools({ sandbox });
