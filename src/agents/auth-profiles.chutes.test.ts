...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.MRBEANBOT_AGENT_DIR;
    else process.env.MRBEANBOT_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-chutes-"));
    process.env.MRBEANBOT_STATE_DIR = tempDir;
    process.env.MRBEANBOT_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
