...
const MODELS_CONFIG: MrBeanBotConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        ...
        const result = await ensureMrBeanBotModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      ...
      await ensureMrBeanBotModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveMrBeanBotAgentDir(), "models.json");
