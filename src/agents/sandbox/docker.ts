...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "MrBeanBot.sandbox=1");
  args.push("--label", `MrBeanBot.sessionKey=${params.scopeKey}`);
  args.push("--label", `MrBeanBot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `MrBeanBot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "MrBeanBot.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`MrBeanBot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`MrBeanBot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("MrBeanBot sandbox recreate --all");
}
