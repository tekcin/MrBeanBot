...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("MrBeanBot models auth setup-token")}\` or \`${formatCliCommand("MrBeanBot configure")}\`.`
          : `Run \`${formatCliCommand("MrBeanBot configure")}\` or set an API key env var.`;
