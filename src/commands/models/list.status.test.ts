...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMrBeanBotAgentDir: vi.fn().mockReturnValue("/tmp/MrBeanBot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/MrBeanBot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveMrBeanBotAgentDir: mocks.resolveMrBeanBotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/MrBeanBot-agent/auth-profiles.json");
