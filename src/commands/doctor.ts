...
function resolveMode(cfg: MrBeanBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("MrBeanBot doctor");

  const root = await resolveMrBeanBotPackageRoot({
  ...
  let cfg: MrBeanBotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("MrBeanBot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("MrBeanBot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("MrBeanBot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("MrBeanBot doctor --fix")}" to apply changes.`);
  }
