...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "MRBEANBOT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MRBEANBOT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MRBEANBOT_GATEWAY_TOKEN");
    expect(message).not.toContain("MRBEANBOT_GATEWAY_PASSWORD");
  });
