...
import type { MrBeanBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: MrBeanBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("MrBeanBot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("MrBeanBot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("MrBeanBot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("MrBeanBot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("MrBeanBot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
