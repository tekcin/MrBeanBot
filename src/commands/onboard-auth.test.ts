...
const requireAgentDir = () => {
  const agentDir = process.env.MRBEANBOT_AGENT_DIR;
  if (!agentDir) throw new Error("MRBEANBOT_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.MRBEANBOT_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.MRBEANBOT_AGENT_DIR;
    } else {
      process.env.MRBEANBOT_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.MRBEANBOT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under MRBEANBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-oauth-"));
    process.env.MRBEANBOT_STATE_DIR = tempStateDir;
    process.env.MRBEANBOT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.MRBEANBOT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MRBEANBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-minimax-"));
    process.env.MRBEANBOT_STATE_DIR = tempStateDir;
    process.env.MRBEANBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.MRBEANBOT_AGENT_DIR;
