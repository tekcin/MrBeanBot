...
export function listAgentEntries(cfg: MrBeanBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: MrBeanBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: MrBeanBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MrBeanBotConfig): AgentSummary[] {
