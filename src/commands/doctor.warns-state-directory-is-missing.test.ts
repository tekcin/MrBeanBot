...
beforeEach(() => {
  ...
  resolveMrBeanBotPackageRoot.mockReset().mockResolvedValue(null);
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.MRBEANBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.MRBEANBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.MRBEANBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveMrBeanBotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadMrBeanBotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "MrBeanBot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.MRBEANBOT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when MRBEANBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/MrBeanBot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.MRBEANBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
