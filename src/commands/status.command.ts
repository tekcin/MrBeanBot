...
export async function statusCommand(
  ...
  runtime.log(theme.heading("MrBeanBot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("MrBeanBot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("MrBeanBot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.mrbeanbot.com/faq");
  runtime.log("Troubleshooting: https://docs.mrbeanbot.com/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("MrBeanBot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("MrBeanBot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("MrBeanBot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("MrBeanBot gateway probe")}`);
  }
}
