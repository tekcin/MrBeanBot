...
function stripUnknownConfigKeys(config: MrBeanBotConfig): {
  config: MrBeanBotConfig;
  removed: string[];
} {
  const parsed = MrBeanBotSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as MrBeanBotConfig;
...
function noteOpencodeProviderOverrides(cfg: MrBeanBotConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: MrBeanBotConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as MrBeanBotConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("MrBeanBot doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("MrBeanBot doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "MrBeanBot doctor --fix" to remove these keys.');
    }
  }
