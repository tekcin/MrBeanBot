...
const execHostEnforced = process.env.MRBEANBOT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.MRBEANBOT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureMrBeanBotCliOnPath({ pathEnv: process.env.PATH ?? "" });
