...
import { loadMrBeanBotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.MRBEANBOT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `MrBeanBot-plugin-${randomUUID()}`);
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.MRBEANBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadMrBeanBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.MRBEANBOT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadMrBeanBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadMrBeanBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
