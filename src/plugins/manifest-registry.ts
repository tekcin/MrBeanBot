...
import { discoverMrBeanBotPlugins, type PluginCandidate } from "./discovery.js";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MRBEANBOT_PLUGIN_MANIFEST_CACHE_MS?.trim();
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.MRBEANBOT_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
