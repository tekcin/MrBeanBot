...
import { discoverMrBeanBotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: MrBeanBotPluginDefinition;
  register?: MrBeanBotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as MrBeanBotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as MrBeanBotPluginDefinition;
...
export function loadMrBeanBotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverMrBeanBotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: MrBeanBotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as MrBeanBotPluginModule;
    } catch (err) {
