...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.MRBEANBOT_STATE_DIR;
  previousConfigPath = process.env.MRBEANBOT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.MRBEANBOT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.MRBEANBOT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.MRBEANBOT_SKIP_CANVAS_HOST;
...
function applyGatewaySkipEnv() {
  process.env.MRBEANBOT_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.MRBEANBOT_STATE_DIR;
    else process.env.MRBEANBOT_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.MRBEANBOT_CONFIG_PATH;
    else process.env.MRBEANBOT_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.MRBEANBOT_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.MRBEANBOT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.MRBEANBOT_SKIP_GMAIL_WATCHER;
    else process.env.MRBEANBOT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.MRBEANBOT_SKIP_CANVAS_HOST;
    else process.env.MRBEANBOT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.MRBEANBOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MRBEANBOT_GATEWAY_TOKEN = fallbackToken;
  }
