...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.MRBEANBOT_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "mrbeanbot");
      ...
      try {
        ...
        process.env.MRBEANBOT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-wizard-home-"));
    process.env.HOME = tempHome;
