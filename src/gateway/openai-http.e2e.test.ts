    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "MrBeanBot", messages: [{ role: "user", content: "hi" }] },
          { "x-MrBeanBot-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-MrBeanBot-agent-id": "beta",
            "x-MrBeanBot-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
