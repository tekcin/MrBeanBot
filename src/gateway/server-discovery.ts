...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "MrBeanBot";
  if (/MrBeanBot/i.test(trimmed)) return trimmed;
  return `${trimmed} (MrBeanBot)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.MRBEANBOT_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "MrBeanBot");
  ...
  const binCli = path.join(cwd, "bin", "MrBeanBot.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.MRBEANBOT_TAILNET_DNS?.trim();
