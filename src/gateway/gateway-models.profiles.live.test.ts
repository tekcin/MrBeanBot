...
import type { MrBeanBotConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.MRBEANBOT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.MRBEANBOT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.MRBEANBOT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: MrBeanBotConfig;
  agentDir: string;
}): MrBeanBotConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<MrBeanBotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.MRBEANBOT_AGENT_DIR,
  ...
  const hostAgentDir = resolveMrBeanBotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-live-state-"));
  ...
  process.env.MRBEANBOT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.MrBeanBot-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveMrBeanBotAgentDir();
  const sanitizedCfg: MrBeanBotConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "MrBeanBot live tool probe (local, safe): " +
    ...
    process.env.MRBEANBOT_CONFIG_PATH = previous.configPath;
    process.env.MRBEANBOT_GATEWAY_TOKEN = previous.token;
    process.env.MRBEANBOT_SKIP_CHANNELS = previous.skipChannels;
    process.env.MRBEANBOT_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.MRBEANBOT_SKIP_CRON = previous.skipCron;
    process.env.MRBEANBOT_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.MRBEANBOT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.MRBEANBOT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.MRBEANBOT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.MrBeanBot-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
