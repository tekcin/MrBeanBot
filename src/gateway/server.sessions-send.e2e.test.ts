...
import { createMrBeanBotTools } from "../agents/moltbot-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.MRBEANBOT_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.MRBEANBOT_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createMrBeanBotTools().find((candidate) => candidate.name === "sessions_send");
