...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of MrBeanBot")).toBe("friends-of-mrbeanbot");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of MrBeanBot"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-mrbeanbot" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-mrbeanbot" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of MrBeanBot"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-mrbeanbot");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-mrbeanbot": { slug: "friends-of-mrbeanbot" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of MrBeanBot"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["mrbeanbot-dm"],
        channelId: "1",
        channelName: "MrBeanBot DM",
        channelSlug: "mrbeanbot-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
