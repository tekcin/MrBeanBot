...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.mrbeanbot.com/cli/dns")}\n`,
    );

  dns
    .command("setup")
    .description(
      "Set up CoreDNS to serve MrBeanBot.internal for unicast DNS-SD (Wide-Area Bonjour)",
    )
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): MrBeanBot.internal"));
      ...
      const serverPath = path.join(confDir, "MrBeanBot.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by MrBeanBot dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
