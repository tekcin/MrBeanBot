...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(resolveMrBeanBotPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("MrBeanBot update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveMrBeanBotPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "MrBeanBot-update-wizard-"));
    const previousGitDir = process.env.MRBEANBOT_GIT_DIR;
    try {
      setTty(true);
      process.env.MRBEANBOT_GIT_DIR = tempDir;
      ...
      process.env.MRBEANBOT_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
