...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    ...
    const env = {
      ...process.env,
      MRBEANBOT_NO_RESPAWN: "1",
      MRBEANBOT_STATE_DIR: stateDir,
      MRBEANBOT_SKIP_CHANNELS: "1",
      MRBEANBOT_SKIP_GMAIL_WATCHER: "1",
      MRBEANBOT_SKIP_CRON: "1",
      MRBEANBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
      MRBEANBOT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "MrBeanBot-entry-bootstrap.mjs");
