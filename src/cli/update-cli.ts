...
const STEP_LABELS: Record<string, string> = {
  ...
  "MrBeanBot doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "Updated successfully. New version, same great assistant.",
...
const DEFAULT_PACKAGE_NAME = "MrBeanBot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "MrBeanBot"]);
const CLI_NAME = resolveCliName();
const MRBEANBOT_REPO_URL = "https://github.com/MrBeanBot/MrBeanBot.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "MrBeanBot");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("MrBeanBot@")) return trimmed.slice("MrBeanBot@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.MRBEANBOT_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MRBEANBOT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MRBEANBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set MRBEANBOT_GIT_DIR to an empty folder or a MrBeanBot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MRBEANBOT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveMrBeanBotPackageRoot({
  ...
  defaultRuntime.log(theme.heading("MrBeanBot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating MrBeanBot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this MrBeanBot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("MrBeanBot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("MrBeanBot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g MrBeanBot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g MrBeanBot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        try {
          ...
          delete process.env.MRBEANBOT_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("MrBeanBot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("MrBeanBot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("MrBeanBot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("MrBeanBot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `MrBeanBot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `MRBEANBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set MRBEANBOT_GIT_DIR to an empty folder or a MrBeanBot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via MRBEANBOT_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update MrBeanBot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["MrBeanBot update", "Update a source checkout (git)"],
        ["MrBeanBot update --channel beta", "Switch to beta channel (git + npm)"],
        ["MrBeanBot update --channel dev", "Switch to dev channel (git + npm)"],
        ["MrBeanBot update --tag beta", "One-off update to a dist-tag or version"],
        ["MrBeanBot update --no-restart", "Update without restarting the service"],
        ["MrBeanBot update --json", "Output result as JSON"],
        ["MrBeanBot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["MrBeanBot update wizard", "Interactive update wizard"],
        ["MrBeanBot --update", "Shorthand for MrBeanBot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run MrBeanBot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.mrbeanbot.com/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["MrBeanBot update status", "Show channel + version status."],
          ["MrBeanBot update status --json", "JSON output."],
          ["MrBeanBot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.mrbeanbot.com/cli/update")}`,
    )
    .action(async (opts) => {
      try {
