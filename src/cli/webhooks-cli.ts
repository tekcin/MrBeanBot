...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.mrbeanbot.com/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + MrBeanBot hooks")
    ...
    .option("--hook-url <url>", "MrBeanBot hook URL")
    .option("--hook-token <token>", "MrBeanBot hook token")
