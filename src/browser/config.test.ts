...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const mrbeanbot = resolveProfile(resolved, "mrbeanbot");
    expect(mrbeanbot?.driver).toBe("mrbeanbot");
    expect(mrbeanbot?.cdpPort).toBe(18800);
    expect(mrbeanbot?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from MRBEANBOT_GATEWAY_PORT when unset", () => {
    const prev = process.env.MRBEANBOT_GATEWAY_PORT;
    process.env.MRBEANBOT_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const mrbeanbot = resolveProfile(resolved, "mrbeanbot");
      expect(mrbeanbot?.cdpPort).toBe(19012);
      expect(mrbeanbot?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.MRBEANBOT_GATEWAY_PORT;
      } else {
        process.env.MRBEANBOT_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const mrbeanbot = resolveProfile(resolved, "mrbeanbot");
      expect(mrbeanbot?.cdpPort).toBe(19022);
      expect(mrbeanbot?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.MRBEANBOT_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "mrbeanbot");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        mrbeanbot: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("mrbeanbot");
  });
});
