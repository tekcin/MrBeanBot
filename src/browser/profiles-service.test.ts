...
vi.mock("./chrome.js", () => ({
  resolveMrBeanBotUserDataDir: vi.fn(() => "/tmp/mrbeanbot-test/mrbeanbot/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "mrbeanbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveMrBeanBotUserDataDir).mockReturnValue(userDataDir);
