...
function setSlackDmPolicy(cfg: MrBeanBotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "MrBeanBot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for MrBeanBot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: MrBeanBotConfig, allowFrom: string[]): MrBeanBotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
