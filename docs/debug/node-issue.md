...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in MrBeanBot
---

# Node + tsx "__name is not a function" crash

## Summary
Running MrBeanBot via Node with `tsx` fails at startup with:

```
[MrBeanBot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `MrBeanBot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch mrbeanbot.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node mrbeanbot.mjs status` works on Node 25.
