...
- Jobs persist under `~/.MrBeanBot/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.MrBeanBot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.MrBeanBot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.MrBeanBot/cron/jobs.json",
...
MrBeanBot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
MrBeanBot cron edit <jobId> --agent ops
MrBeanBot cron edit <jobId> --clear-agent
```
```
...
```bash
MrBeanBot system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`MrBeanBot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `MRBEANBOT_SKIP_CRON`.
