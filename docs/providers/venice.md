---
summary: "Use Venice AI privacy-focused models in MrBeanBot"
read_when:
  - You want privacy-focused inference in MrBeanBot
...
```bash
MrBeanBot onboard --auth-choice venice-api-key
```
...
```bash
MrBeanBot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, MrBeanBot shows all available Venice models. Pick based on your needs:
...
```bash
MrBeanBot models set venice/claude-opus-45
MrBeanBot models set venice/llama-3.3-70b
```

List all available models:

```bash
MrBeanBot models list | grep venice
```

## Configure via `MrBeanBot configure`

1. Run `MrBeanBot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
MrBeanBot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
MrBeanBot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
MrBeanBot chat --model venice/claude-opus-45

# Use uncensored model
MrBeanBot chat --model venice/venice-uncensored

# Use vision model with image
MrBeanBot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
MrBeanBot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `MrBeanBot models list` to see currently available models. Some models may be temporarily offline.
