---
summary: "Use Synthetic's Anthropic-compatible API in MrBeanBot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. MrBeanBot registers it as the
...
```bash
MrBeanBot onboard --auth-choice synthetic-api-key
```
...
Note: MrBeanBot's Anthropic client appends `/v1` to the base URL, so use
