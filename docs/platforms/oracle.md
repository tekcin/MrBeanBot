---
summary: "MrBeanBot on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up MrBeanBot on Oracle Cloud
  - Looking for low-cost VPS hosting for MrBeanBot
  - Want 24/7 MrBeanBot on a small server
---

# MrBeanBot on Oracle Cloud (OCI)

## Goal

Run a persistent MrBeanBot Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for MrBeanBot (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname MrBeanBot
...
sudo tailscale up --ssh --hostname=MrBeanBot
```

This enables Tailscale SSH, so you can connect via `ssh MrBeanBot` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@MrBeanBot` (or use the Tailscale IP).
...
MrBeanBot config set gateway.auth.mode token
MrBeanBot doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
MrBeanBot config set gateway.tailscale.mode serve
MrBeanBot config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart MrBeanBot-gateway
```
...
```
https://MrBeanBot.<tailnet-name>.ts.net/
```
...
This setup often removes the *need* for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `MrBeanBot security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.MrBeanBot`
- **Security audit:** `MrBeanBot security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@MrBeanBot
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=MrBeanBot --reset
```
...
journalctl --user -u MrBeanBot-gateway -n 50
```
