---
summary: "MrBeanBot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up MrBeanBot on a Raspberry Pi
  - Running MrBeanBot on ARM devices
  - Building a cheap always-on personal AI
---

# MrBeanBot on Raspberry Pi

## Goal

Run a persistent, always-on MrBeanBot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most MrBeanBot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled MrBeanBot

# Enable if not
sudo systemctl enable MrBeanBot

# Start on boot
sudo systemctl start MrBeanBot
```
...
```bash
# Check logs
journalctl -u MrBeanBot --no-pager -n 100

# Common fix: rebuild
cd ~/MrBeanBot  # if using hackable install
npm run build
sudo systemctl restart MrBeanBot
```
