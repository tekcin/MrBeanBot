...
read_when:
  - Installing MrBeanBot on Windows
  - Looking for Windows companion app status
---
# Windows (WSL2)

MrBeanBot on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- Remote nodes must point at a **reachable** Gateway URL (not `127.0.0.1`); use
  `MrBeanBot status --all` to confirm.
...
### 3) Install MrBeanBot (inside WSL)
