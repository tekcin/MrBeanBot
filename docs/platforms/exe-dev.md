---
summary: "Run MrBeanBot Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: MrBeanBot Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install MrBeanBot  
2) Run `MrBeanBot onboard --install-daemon`  
...
```bash
ssh exe.dev new --name=MrBeanBot
```
...
```bash
ssh -N -L 18789:127.0.0.1:18789 MrBeanBot.exe.xyz
```
...
```bash
export MRBEANBOT_GATEWAY_TOKEN="$(openssl rand -hex 32)"
MrBeanBot gateway --bind lan --port 8080 --token "$MRBEANBOT_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.MrBeanBot/MrBeanBot.json`:
...
Notes:
- Non-loopback binds require `gateway.auth.token` (or `MRBEANBOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is only for remote CLI calls; it does not enable local auth.

Then point exe.dev’s proxy at `8080` (or whatever port you chose) and open your VM’s HTTPS URL:

```bash
ssh exe.dev share port MrBeanBot 8080
```

Open:
- `https://MrBeanBot.exe.xyz/`
...
On Linux, MrBeanBot uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status MrBeanBot-gateway[-<profile>].service
```
