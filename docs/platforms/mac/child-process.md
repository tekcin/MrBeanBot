...
service via the external `MrBeanBot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`MRBEANBOT_PROFILE`; legacy `com.MrBeanBot.*` is supported).
