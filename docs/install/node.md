...
read_when:
  - "You installed MrBeanBot but `MrBeanBot` is “command not found”"
...
MrBeanBot’s runtime baseline is **Node 22+**.

If you can run `npm install -g MrBeanBot@latest` but later see `MrBeanBot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
