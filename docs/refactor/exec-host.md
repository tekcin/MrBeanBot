...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.MrBeanBot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.MrBeanBot/node.json` (node id + pairing token).
...
Path: `~/.MrBeanBot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.MrBeanBot/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.MrBeanBot/exec-approvals.sock` (0600).
