---
summary: "First-run onboarding flow for MrBeanBot (macOS app)"
read_when:
...
- Writes credentials to `~/.MrBeanBot/credentials/oauth.json`
...
The app can install the global `MrBeanBot` CLI via npm/pnpm so terminal
...
```bash
MrBeanBot webhooks gmail setup --account you@gmail.com
```
...
- `~/.MrBeanBot/credentials/oauth.json`
