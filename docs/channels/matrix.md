...
Matrix is an open, decentralized messaging protocol. MrBeanBot connects as a Matrix **user**
...
```bash
MrBeanBot plugins install @MrBeanBot/matrix
```

Local checkout (when running from a git repo):

```bash
MrBeanBot plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `MrBeanBot plugins install @MrBeanBot/matrix`
   - From a local checkout: `MrBeanBot plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: MrBeanBot calls the same
     login endpoint, stores the access token in `~/.MrBeanBot/credentials/matrix/credentials.json`,
...
- On first connection, MrBeanBot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  MrBeanBot logs a warning.
...
`~/.MrBeanBot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `MrBeanBot pairing list matrix`
  - `MrBeanBot pairing approve matrix <CODE>`
...
- On startup, MrBeanBot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
