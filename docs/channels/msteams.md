...
```bash
MrBeanBot plugins install @MrBeanBot/msteams
```

Local checkout (when running from a git repo):
```bash
MrBeanBot plugins install ./extensions/msteams
```
...
3) Configure MrBeanBot with those credentials.
...
## Goals
- Talk to MrBeanBot via Teams DMs, group chats, or channels.
...
- On startup, MrBeanBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.MrBeanBot/MrBeanBot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring MrBeanBot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `MrBeanBot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `MrBeanBot plugins install @MrBeanBot/msteams`
   - From a local checkout: `MrBeanBot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "MrBeanBot in Teams", "full": "MrBeanBot in Teams" },
...
MrBeanBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, MrBeanBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/MrBeanBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
MrBeanBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `MrBeanBot message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.MrBeanBot/msteams-polls.json`.
...
**CLI:**
```bash
MrBeanBot message send --channel msteams \
...
MrBeanBot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
MrBeanBot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
MrBeanBot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
MrBeanBot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
