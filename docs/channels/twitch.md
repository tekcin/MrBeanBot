...
read_when:
  - Setting up Twitch chat integration for MrBeanBot
---
# Twitch (plugin)

Twitch chat support via IRC connection. MrBeanBot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
MrBeanBot plugins install @MrBeanBot/twitch
```

Local checkout (when running from a git repo):

```bash
MrBeanBot plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `MRBEANBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "MrBeanBot",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use MRBEANBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
MRBEANBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
