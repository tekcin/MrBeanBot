...
```bash
MrBeanBot plugins install @MrBeanBot/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
MrBeanBot plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "MrBeanBot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `MrBeanBot pairing list nextcloud-talk`
  - `MrBeanBot pairing approve nextcloud-talk <CODE>`
