...
Tlon is a decentralized messenger built on Urbit. MrBeanBot connects to your Urbit ship and can
...
```bash
MrBeanBot plugins install @MrBeanBot/tlon
```

Local checkout (when running from a git repo):

```bash
MrBeanBot plugins install ./extensions/tlon
```
...
Use these with `MrBeanBot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, MrBeanBot replies in-thread.
