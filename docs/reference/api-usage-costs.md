...
MrBeanBot features that can generate provider usage or paid API calls.
...
- `MrBeanBot status --usage` and `MrBeanBot channels list` show provider **usage windows**
...
MrBeanBot can pick up credentials from:
...
- `MrBeanBot status --usage`
- `MrBeanBot models status --json`
...
### 8) Model scan / probe
`MrBeanBot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
