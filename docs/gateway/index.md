...
- Replaces the legacy `gateway` command. CLI entry point: `MrBeanBot gateway`.
...
MrBeanBot gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.MrBeanBot/MrBeanBot.json` (or `MRBEANBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__MrBeanBot__/canvas/` from `~/mrbeanbot/canvas`. Disable with `canvasHost.enabled=false` or `MRBEANBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `MRBEANBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `MRBEANBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.MrBeanBot.*` may still exist)
- Linux: `MrBeanBot-gateway-<profile>.service`
- Windows: `MrBeanBot Gateway (<profile>)`

Install metadata is embedded in the service config:
- `MRBEANBOT_SERVICE_MARKER=MrBeanBot`
- `MRBEANBOT_SERVICE_KIND=gateway`
- `MRBEANBOT_SERVICE_VERSION=<version>`
...
```bash
MrBeanBot --dev setup
MrBeanBot --dev gateway --allow-unconfigured
# then target the dev instance:
MrBeanBot --dev status
MrBeanBot --dev health
```

Defaults (can be overridden via env/flags/config):
- `MRBEANBOT_STATE_DIR=~/.MrBeanBot-dev`
- `MRBEANBOT_CONFIG_PATH=~/.MrBeanBot-dev/MrBeanBot.json`
- `MRBEANBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/mrbeanbot-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `MRBEANBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `MRBEANBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `MRBEANBOT_CONFIG_PATH`
- unique `MRBEANBOT_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
MrBeanBot --profile main gateway install
MrBeanBot --profile rescue gateway install
```

Example:
```bash
MRBEANBOT_CONFIG_PATH=~/.MrBeanBot/a.json MRBEANBOT_STATE_DIR=~/.MrBeanBot-a MrBeanBot gateway --port 19001
MRBEANBOT_CONFIG_PATH=~/.MrBeanBot/b.json MRBEANBOT_STATE_DIR=~/.MrBeanBot-b MrBeanBot gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `MrBeanBot health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `MrBeanBot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `MrBeanBot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.MrBeanBot.*` is cleaned up).
  - `MrBeanBot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are MrBeanBot profile services.
  ...
  - Cleanup: `MrBeanBot gateway uninstall` (current service) and `MrBeanBot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `MrBeanBot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- MrBeanBot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.MrBeanBot.*` labels still unload cleanly).
- To stop it cleanly, use `MrBeanBot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `MrBeanBot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `MrBeanBot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
MrBeanBot installs a **systemd user service** by default on Linux/WSL2. We
...
`MrBeanBot gateway install` writes the user unit. `MrBeanBot doctor` audits the
...
Restart=always
RestartSec=5
Environment=MRBEANBOT_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/MrBeanBot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now MrBeanBot-gateway[-<profile>].service
```
...
## CLI helpers
- `MrBeanBot gateway health|status` — request health/status over the Gateway WS.
- `MrBeanBot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `MrBeanBot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `MrBeanBot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `MrBeanBot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `MrBeanBot gateway` and the legacy TCP control port.
