...
`MrBeanBot doctor` is the repair + migration tool for MrBeanBot. It fixes stale
...
- Extra workspace dir detection (`~/MrBeanBot`).
...
you to run `MrBeanBot doctor`.

Doctor will:
- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.MrBeanBot/MrBeanBot.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.MrBeanBot/sessions/` to `~/.MrBeanBot/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.MrBeanBot/agent/` to `~/.MrBeanBot/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.MrBeanBot/credentials/*.json` (except `oauth.json`)
  - to `~/.MrBeanBot/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `MrBeanBot doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.MrBeanBot` folders exist across
  home directories or when `MRBEANBOT_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.MrBeanBot/MrBeanBot.json` is
...
offers to remove them and install the MrBeanBot service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named MrBeanBot gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `MrBeanBot doctor --generate-gateway-token` to force token
...
Notes:
- `MrBeanBot doctor` prompts before rewriting supervisor config.
- `MrBeanBot doctor --yes` accepts the default repair prompts.
- `MrBeanBot doctor --repair` applies recommended fixes without prompts.
- `MrBeanBot doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `MrBeanBot gateway install --force`.
