...
- **gateway host** → `MrBeanBot` process on the gateway machine
...
`~/.MrBeanBot/exec-approvals.json` directly.

CLI: `MrBeanBot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
