---
summary: "Agent tool surface for MrBeanBot (browser, canvas, nodes, message, cron) replacing legacy `MrBeanBot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `MrBeanBot-*` skills
---

# Tools (MrBeanBot)

MrBeanBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `MrBeanBot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `MrBeanBot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, MrBeanBot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`MrBeanBot node run`).
...
Notes:
- Requires a Brave API key (recommended: `MrBeanBot configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated mrbeanbot browser.
...
- Quick smoke: `MrBeanBot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `MrBeanBot gateway` restart in-place)
