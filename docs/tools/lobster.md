---
title: Lobster
summary: "Typed workflow runtime for MrBeanBot with resumable approval gates."
description: Typed workflow runtime for MrBeanBot — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets MrBeanBot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: MrBeanBot runs one Lobster tool call and gets a structured result.
...
MrBeanBot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | mrbeanbot.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In MrBeanBot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the MrBeanBot Gateway (see the [Lobster repo](https://github.com/MrBeanBot/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), MrBeanBot keeps core tools enabled. To restrict core
...
→ mrbeanbot calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ mrbeanbot calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls mrbeanbot tools that do.
