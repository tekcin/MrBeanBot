---
summary: "Chrome extension: let MrBeanBot drive your existing Chrome tab"
read_when:
...
MrBeanBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the MrBeanBot release (npm package) as static files. There is no separate “build” step.

After upgrading MrBeanBot:
- Re-run `MrBeanBot browser extension install` to refresh the installed files under your MrBeanBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

MrBeanBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want MrBeanBot to control.
...
- `ON`: attached; MrBeanBot can drive that tab.
...
Debugging: `MrBeanBot sandbox explain`
...
`MrBeanBot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `MrBeanBot browser extension install` first to copy the extension to a stable location under your MrBeanBot state directory.
