...
Deploy MrBeanBot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/MrBeanBot/MrBeanBot" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run MrBeanBot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: MRBEANBOT_STATE_DIR
        value: /data/.MrBeanBot
      - key: MRBEANBOT_WORKSPACE_DIR
        value: /data/workspace
      - key: MRBEANBOT_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/MrBeanBot`.
...
If you use the original MrBeanBot repository, Render will not auto-deploy your MrBeanBot. To update it, run a manual Blueprint sync from the dashboard.
...
For MrBeanBot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any MrBeanBot host.
