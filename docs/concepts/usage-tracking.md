...
- `/usage cost` in chats: local cost summary aggregated from MrBeanBot session logs.
- CLI: `MrBeanBot status --usage` prints a full per-provider breakdown.
- CLI: `MrBeanBot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
