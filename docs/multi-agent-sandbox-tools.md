...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `MrBeanBot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `MrBeanBot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
1. **Check agent resolution:**
   ```exec
   MrBeanBot agents list --bindings
   ```

2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=MrBeanBot.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${MRBEANBOT_STATE_DIR:-$HOME/.MrBeanBot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
